...
class Evaluator:
    def __init__(self, dataset: Dataset, input_reader: JsonInputReader, text_encoder: BertTokenizer, logger,
                 no_overlapping: bool, no_partial_overlapping: bool, no_duplicate: bool, predictions_path: str,
                 examples_path: str, example_count: int, epoch: int, dataset_label: str, cls_threshold: float,
                 boundary_threshold: float, save_prediction=False):
    ...
    def eval_batch(self, entity_logits: torch.tensor, p_left: torch.tensor, p_right: torch.tensor, outputs, batch=None):
        ...
        batch_entity_mask = batch_entity_mask * (batch_entity_spans[:, :, 0] <= batch_entity_spans[:, :, 1]) * (
                    batch_entity_left_scores > self._boundary_threshold) * (
                                        batch_entity_right_scores > self._boundary_threshold)
        ...
        for i in range(batch_size):
            ...
            if self._input_reader.entity_type_count < 1000 and self._save_prediction:
                decode_entity = dict(tokens=[t.phrase for t in doc.tokens], pre_entities=[], gt_entities=[],
                                     org_id=doc.doc_id)
                ...
                for entity in doc.entities:
                    ...
                    converted_entity = dict(type=entity_type, start=span_tokens[0].index, end=span_tokens[-1].index,
                ...
                for j in range(entity_left.size(1)):
                    ...
                    decode_entity["pre_entities"].append(
                        dict(entity_left=entity_left[i][j].item(), entity_right=entity_right[i][j].item(),
                             p_left=roundlist(p_left[i][j].tolist()), p_right=roundlist(p_right[i][j].tolist()),
                             phrase=span_tokens,
                             entity_type=self._input_reader.get_entity_type(batch_entity_types[i][j].item()).identifier,
                             entity_prob=roundlist(entity_prob[i][j].tolist())))
            ...
            sample_pred_entities = self._convert_pred_entities(valid_entity_types, valid_entity_spans,
                                                               valid_entity_scores, valid_left_scores,
                                                               valid_right_scores, doc)
    ...
    def compute_scores(self):
        ...
        self._log("")
        gt_wo_type, pred_wo_type = self._convert_by_setting(self._gt_entities, self._pred_entities,
    ...
    def store_predictions(self):
        ...
        for i, doc in enumerate(self._dataset.documents):
            ...
            doc_predictions = dict(tokens=[t.phrase for t in tokens], pre_entities=pre_converted_entities,
                                   gt_entities=gt_converted_entities)
    ...
    def _convert_pred_entities(self, pred_types: torch.tensor, pred_spans: torch.tensor, pred_scores: torch.tensor,
        ...
        for i in range(pred_types.shape[0]):
            ...
            decode_entity["entities"].append(
                {"start": start, "end": end, "entity_type": entity_type.identifier, "cls_score": round(cls_score, 2),
                 "left_score": round(left_score, 2), "right_score": round(right_score, 2)})
