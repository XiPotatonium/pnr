...
class IdentifierLoss(Loss):
    def __init__(self, entity_type_count, device, model, optimizer, scheduler, max_grad_norm, nil_weight,
                 match_class_weight, match_boundary_weight, loss_class_weight, loss_boundary_weight, type_loss, solver,
        ...
        self.criterion = Criterion(entity_type_count, self.weight_dict, nil_weight, losses, type_loss=type_loss,
                                   match_class_weight=match_class_weight, match_boundary_weight=match_boundary_weight,
    ...
    def compute(self, entity_logits, pred_left, pred_right, output, gt_types, gt_spans, entity_masks, epoch,
                deeply_weight="same", seq_logits=None, gt_seq_labels=None, batch=None):
        # set_trace()
        ...
        entity_span_masks = entity_masks.unsqueeze(2).repeat(1, 1, 2)
        spans_wo_nil = gt_spans.masked_select(entity_span_masks).view(-1, 2)

        targets = {"labels": gt_types_wo_nil, "gt_left": spans_wo_nil[:, 0], "gt_right": spans_wo_nil[:, 1],
                   "sizes": sizes}
        ...
        for out_dict in output:
            ...
            outputs = {
                "pred_logits": entity_logits, "pred_left": pred_left, "pred_right": pred_right,
                "token_mask": batch["token_masks"],
                'tag': out_dict.get('tag')  # 这是后面加的一个tag，给proposal层的output一些自定义的loss
            }
            if 'max_gt_len' in out_dict:
                max_gt_len = out_dict['max_gt_len']

                entity_mask_with_max_gt_len = entity_masks.clone().detach()
                chunked_spans_wo_nil = torch.split(spans_wo_nil, sizes, dim=0)
                for bi, batch_spans_wo_nil in enumerate(chunked_spans_wo_nil):
                    for ei, span in enumerate(batch_spans_wo_nil):
                        if span[1] - span[0] + 1 > max_gt_len:
                            # 把超过max_gt_len长度的gt给mask掉
                            entity_mask_with_max_gt_len[bi][ei] = False

                gt_types_wo_nil_with_max_gt_len = gt_types.masked_select(entity_mask_with_max_gt_len)
                sizes_with_max_gt_len = [i.sum() for i in entity_mask_with_max_gt_len]
                entity_mask_with_max_gt_len = entity_mask_with_max_gt_len.unsqueeze(2).repeat(1, 1, 2)
                spans_wo_nil_with_max_gt_len = gt_spans.masked_select(entity_mask_with_max_gt_len).view(-1, 2)

                targets_with_max_gt_len = {
                    "labels": gt_types_wo_nil_with_max_gt_len,
                    "gt_left": spans_wo_nil_with_max_gt_len[:, 0],
                    "gt_right": spans_wo_nil_with_max_gt_len[:, 1],
                    "sizes": sizes_with_max_gt_len
                }
                loss_dict = self.criterion(outputs, targets_with_max_gt_len, epoch)
            else:
...
class Criterion(nn.Module):
    ...
    def __init__(self, entity_type_count, weight_dict, nil_weight, losses, type_loss, match_class_weight,
                 match_boundary_weight, solver, match_warmup_epoch):
    ...
    def loss_labels(self, outputs, targets, indices, num_spans):
        ...
        if self.type_loss == "bceloss":
            src_logits = src_logits.view(-1, src_logits.size(2))
            target_classes = target_classes.view(-1)
            target_classes_onehot = torch.zeros([target_classes.size(0), src_logits.size(1)], dtype=torch.float32).to(
    ...
    def loss_boundary(self, outputs, targets, indices, num_spans):
        ...
        left_onehot = torch.zeros([target_spans_left.size(0), src_spans_left.size(1)], dtype=torch.float32).to(
            device=target_spans_left.device)
        left_onehot.scatter_(1, target_spans_left.unsqueeze(1), 1)

        right_onehot = torch.zeros([target_spans_right.size(0), src_spans_right.size(1)], dtype=torch.float32).to(
            device=target_spans_right.device)
